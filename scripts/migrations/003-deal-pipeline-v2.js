/**
 * Migration: Upgrade default deal pipeline to 12 production stages.
 */
export const id = '003-deal-pipeline-v2';
export const description = 'Upgrade legacy 5-stage pipelines to 12-stage production default';

const NEW_STAGES = [
  { key: 'new_lead', label: 'New Lead', order: 0, color: '#94a3b8', probability: 5 },
  { key: 'qualified', label: 'Qualified', order: 1, color: '#6366f1', probability: 15 },
  { key: 'first_contact', label: 'First Contact', order: 2, color: '#818cf8', probability: 20 },
  { key: 'demo_scheduled', label: 'Demo Scheduled', order: 3, color: '#3b82f6', probability: 30 },
  { key: 'demo_completed', label: 'Demo Completed', order: 4, color: '#0ea5e9', probability: 40 },
  { key: 'quotation_sent', label: 'Quotation Sent', order: 5, color: '#8b5cf6', probability: 50 },
  { key: 'follow_up', label: 'Follow-up', order: 6, color: '#a855f7', probability: 55 },
  { key: 'negotiation', label: 'Negotiation', order: 7, color: '#f59e0b', probability: 70 },
  { key: 'decision_pending', label: 'Decision Pending', order: 8, color: '#f97316', probability: 80 },
  { key: 'payment_pending', label: 'Payment Pending', order: 9, color: '#eab308', probability: 90 },
  { key: 'won', label: 'Won', order: 10, color: '#10b981', probability: 100, isWon: true },
  { key: 'lost', label: 'Lost', order: 11, color: '#ef4444', probability: 0, isLost: true },
];

const STAGE_MAP = {
  qualification: 'new_lead',
  proposal: 'quotation_sent',
  negotiation: 'negotiation',
  closed_won: 'won',
  closed_lost: 'lost',
};

export async function up(db) {
  const pipelines = db.collection('pipelines');
  const deals = db.collection('deals');

  const legacy = await pipelines.find({
    isDefault: true,
    entityType: 'deal',
    'stages.key': { $in: ['qualification', 'closed_won'] },
  }).toArray();

  for (const pipe of legacy) {
    await pipelines.updateOne({ _id: pipe._id }, { $set: { stages: NEW_STAGES, updatedAt: new Date() } });
    for (const [oldKey, newKey] of Object.entries(STAGE_MAP)) {
      await deals.updateMany(
        { businessId: pipe.businessId, stage: oldKey },
        { $set: { stage: newKey, pipelineId: pipe._id } }
      );
    }
    console.log(`  Upgraded pipeline ${pipe._id} for business ${pipe.businessId}`);
  }
}

export async function down(db) {
  // No down migration — businesses may have customized stages
}
