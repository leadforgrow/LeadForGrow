...
import { isLegacyPipeline, LEGACY_DEAL_STAGE_MAP, LEGACY_STAGE_MAP } from '@/lib/crm/stageKeys';

const ALL_LEGACY_STAGE_MAP = { ...LEGACY_DEAL_STAGE_MAP, ...LEGACY_STAGE_MAP };

/**
 * Upgrade a legacy default pipeline to the current 8-stage deal pipeline.
 */
async function upgradeLegacyDefaultPipeline(pipeline) {
  ...
  for (const [oldKey, newKey] of Object.entries(ALL_LEGACY_STAGE_MAP)) {
...
export function getStageByKey(pipeline, stageKey) {
  ...
  const mapped = LEGACY_DEAL_STAGE_MAP[stageKey] || LEGACY_STAGE_MAP[stageKey];
