...
/**
 * Send onboarding confirmation email to user via Resend
 */
export const sendUserConfirmationEmail = async (userName, userEmail, meetLink) => {
  try {
    ...
    console.log(`[Onboarding] User confirmation sent to ${userEmail}: ${result.id}`);
    return result;
  } catch (error) {
...
/**
 * Send internal team notification via Resend
 */
export const sendInternalNotification = async (userName, userEmail, userPhone, meetLink, planId = null) => {
  try {
    ...
    const result = await sendResendEmail({
      to: internalEmails,
      from: 'LeadForGrow System <info@leadforgrow.com>',
    ...
    console.log(`[Onboarding] Internal notification sent: ${result.id}`);
    return result;
  } catch (error) {
