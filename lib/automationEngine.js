...
export const automationEngine = {
  ...
  processLeadTrigger: async (lead, trigger) => {
    try {
      ...
      // Fetch ALL rules for business (lean) and filter in memory to avoid query issues
      const allRules = await AutomationRule.find({ businessId: lead.businessId }).lean();
      console.log(`[Engine] DEBUG: Fetched ${allRules.length} raw rules.`);
