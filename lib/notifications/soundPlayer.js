...
/**
 * Install a one-time global listener that "unlocks" the AudioContext on the
 * first user gesture (click, keypress, touch). Browsers require this because
 * of autoplay policies — a page can't just start playing sound on load, it
 * has to have proven the user is present first. After the first click, we
 * can freely play sounds programmatically for the rest of the session.
 *
 * Runs on module load (once) so by the time the first SSE event fires the
 * unlock is already armed.
 */
function installUnlockOnce() {
  ...
  const unlock = () => {
    try {
      ...
      // Play a completely silent buffer — this is the classic Safari trick
      // to force the context out of suspended state during a user gesture.
      if (ctx) {
...
if (typeof window !== 'undefined') installUnlockOnce();
...
function getContext() {
  ...
  if (ctx.state === 'suspended') {
    // Fire-and-forget — if we're mid-user-gesture this succeeds, else no-op
    ctx.resume().catch(() => {});
  }
  return ctx;
}
