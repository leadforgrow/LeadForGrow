...
export const callRulesEngine = {
  ...
  evaluateEligibility: async (businessId) => {
    ...
    // 1. Fetch current usage and business quotas
    const [usage, business] = await Promise.all([
      callRepository.getUsage(businessId, currentMonth),
      Business.findById(businessId).select('quotas')
    ]);
    
    if (!business) {
      return { eligible: false, reason: 'BUSINESS_NOT_FOUND' };
    }
    ...
    // 3. Evaluate against business quotas
    const maxCallbacks = business.quotas?.maxCallbacks || callLimits.DEFAULT_MAX_CALLBACKS_PER_MONTH;
    const maxSeconds = business.quotas?.maxCallSeconds || callLimits.DEFAULT_MAX_TOTAL_SECONDS_PER_MONTH;
