...
export async function createBusinessMailer(business) {
  ...
  const currentKey = process.env.ENCRYPTION_KEY || 'MISSING';
  console.log(`${logPrefix} [DEBUG] ENCRYPTION_KEY Checksum: ${currentKey.substring(0, 4)}...${currentKey.substring(currentKey.length - 4)} (Len: ${currentKey.length})`);
  ...
  const transporter = nodemailer.createTransport({
    ...
    timeout: 8000, // 8s timeout to fail fast before Serverless Function limits (10s) kill it
    pool: false,
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    ...
    try {
      ...
      const finalMailOptions = {
        ...
        ...(mailOptions.cc?.length ? { cc: mailOptions.cc } : {}),
        ...(mailOptions.bcc?.length ? { bcc: mailOptions.bcc } : {}),
        ...(mailOptions.attachments?.length ? { attachments: mailOptions.attachments } : {}),
      };
