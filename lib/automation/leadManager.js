  ...
  async processMetaLead(businessId, leadData) {
    ...
    try {
      ...
      const lead = await Lead.create({
        ...
        phone,
        source: 'facebook_ad',
        sourceDetails: `Meta Lead Form: ${formId}`,
        metaLeadId,
        campaignName,
        adSetName,
        adName,
      ...
      automationEngine.triggerForNewLead(lead, businessId).catch(err => {
        console.error('[LeadManager] Meta Ads Automation Trigger Error:', err);
      });
      ...
      if (targetUserId) {
        ...
        }).catch(e => console.error('[LeadManager] Notification Error:', e.message));
      }

      return { status: 'success', leadId: lead._id };
    } catch (error) {
      console.error('[LeadManager] Meta Lead Processing Error:', error);
      throw error;
    }
  }
};
