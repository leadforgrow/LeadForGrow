...
const getEncryptionKey = () => {
  ...
  if (!key) {
    console.warn('[Encryption] No ENCRYPTION_KEY found in environment, using a stable fallback. (NOT RECOMMENDED FOR PRODUCTION)');
    // 64-character hex string = 32 bytes
    return Buffer.from('8a7b9c1d0e2f3a4b5c6d7e8f9a0b1c2d8a7b9c1d0e2f3a4b5c6d7e8f9a0b1c2d', 'hex');
  }
  ...
  try {
    ...
    console.warn(`[Encryption] Invalid hex key length: ${buffer.length} bytes. Auto-correcting to 32 bytes.`);
    const finalKey = Buffer.alloc(32);
    buffer.copy(finalKey);
    return finalKey;
  } catch (e) {
    console.error('[Encryption] Failed to parse hex key, using fallback');
    return Buffer.from('8a7b9c1d0e2f3a4b5c6d7e8f9a0b1c2d8a7b9c1d0e2f3a4b5c6d7e8f9a0b1c2d', 'hex');
  }
};
