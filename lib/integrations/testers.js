...
async function testHostingerSmtp(credentials) {
  const start = Date.now();
  const username = credentials.username;
  const rawPassword = credentials.password?.includes(':')
  ...
  if (!username || !rawPassword) {
    return {
      success: false,
      message: 'Mailbox username and password are required',
      durationMs: Date.now() - start,
    };
  }

  const host =
    credentials.host && !String(credentials.host).includes('titan')
      ? credentials.host
      : 'smtp.hostinger.com';
  ...
  const transporter = nodemailer.createTransport({
    ...
    port,
    secure: port === 465,
    auth: { user: username, pass: rawPassword },
    requireTLS: port !== 465,
    authMethod: 'LOGIN',
  ...
  try {
    ...
    return {
      success: true,
      message: `SMTP verified — ${username} via ${host}:${port}`,
      durationMs: Date.now() - start,
      accountInfo: { label: username, externalId: username },
    };
  } catch (err) {
    let hint = err.message || 'SMTP verification failed';
    if (err.code === 'EAUTH') {
      hint =
        'Authentication failed. Double-check the mailbox email + password. If you use an alias, log in with the primary mailbox.';
    } else if (err.code === 'ETIMEDOUT' || err.code === 'ECONNECTION') {
      hint = `Could not reach ${host}:${port}. Check the host/port and your network.`;
    }
    return { success: false, message: hint, durationMs: Date.now() - start };
  }
}
...
export async function testIntegration(integrationId, credentials, business) {
  ...
  try {
    switch (integrationId) {
      ...
      case 'hostinger-smtp':
        return await testHostingerSmtp(credentials);
      case 'meta-ads':
...
async function testMetaAds(credentials) {
  const start = Date.now();
  const { validateMetaPageToken, resolveLongLivedPageToken, META_TOKEN_EXPIRED_USER_MESSAGE } = await import('@/lib/meta/token');

  let token = credentials.accessToken?.includes(':') ? decrypt(credentials.accessToken) : credentials.accessToken;
  const pageId = credentials.pageId;
  const appId = credentials.appId;
  const appSecret = credentials.appSecret?.includes(':') ? decrypt(credentials.appSecret) : credentials.appSecret;

  if (!token || !pageId) {
    return { success: false, message: 'Page ID and Access Token are required', durationMs: Date.now() - start };
  }

  const resolved = await resolveLongLivedPageToken({ accessToken: token, pageId, appId, appSecret });
  if (resolved.exchanged && resolved.accessToken) {
    token = resolved.accessToken;
  }

  try {
    const data = await validateMetaPageToken(pageId, token);
    return {
      success: true,
      message: `Connected to page: ${data.name}${resolved.exchanged ? ' (long-lived token saved)' : ''}`,
      durationMs: Date.now() - start,
      accountInfo: { label: data.name, externalId: data.id },
      refreshedToken: resolved.exchanged ? token : undefined
    };
  } catch (error) {
    if (error.tokenExpired) {
      return {
        success: false,
        message: META_TOKEN_EXPIRED_USER_MESSAGE,
        tokenExpired: true,
        durationMs: Date.now() - start
      };
    }
    return {
      success: false,
      message: error.message || 'Meta Ads validation failed',
      durationMs: Date.now() - start
    };
  }
}
...
export async function syncIntegration(integrationId, integration, business) {
  ...
  switch (integrationId) {
    case 'meta-ads':
      return syncMetaAds(integration, business, start);
    default:
      return {
        success: true,
...
async function syncMetaAds(integration, business, start) {
  ...
  if (!pageId || !accessToken) {
    metaWarn('Sync', 'Missing pageId or accessToken — aborting sync');
    return {
      success: false,
      message: 'Page ID and Page Access Token are required to sync leads',
      recordsProcessed: 0,
      durationMs: Date.now() - start
    };
  }
  ...
  try {
    const result = await syncMetaLeadsFromLeadCenter(
      business._id.toString(),
      pageId,
      accessToken
    );

    metaLog('Sync', 'syncMetaLeadsFromLeadCenter finished', result);
    ...
    metaError('Sync', 'syncMetaAds failed', error);
    const { MetaGraphError, META_TOKEN_EXPIRED_USER_MESSAGE } = await import('@/lib/meta/token');
    const tokenExpired = error instanceof MetaGraphError && error.tokenExpired;
    return {
      success: false,
      message: tokenExpired ? META_TOKEN_EXPIRED_USER_MESSAGE : error.message || 'Meta Lead Center sync failed',
      tokenExpired,
      recordsProcessed: 0,
      durationMs: Date.now() - start
    };
