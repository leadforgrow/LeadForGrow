...
export const sendAutoWhatsApp = async (lead, business, template, templateName = null, headerMedia = null) => {
  ...
  try {
    ...
    if (credentials.provider === 'interakt') {
      ...
      // ASYNC PERSISTENCE: Save record to Message collection (non-blocking)
      ...
      Message.create({
        ...
        direction: 'outgoing',
        text: message,
        externalMessageId: data.id || data.messageId, // Interakt returns ID
        timestamp: new Date()
      }).catch(err => console.error('[WhatsApp] Failed to save outgoing message record:', err));
      ...
      // Save record for simulated Meta send
      const Message = mongoose.models.Message || (await import('@/models/automation/Message')).default;
      Message.create({
        leadId: lead._id,
        businessId: business._id,
        ...
        timestamp: new Date()
      }).catch(err => console.error('[WhatsApp] Failed to save simulated meta message:', err));
