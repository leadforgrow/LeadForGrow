...
export async function recordOutgoingMessage(leadId, businessId, text, externalId, templateName = null) {
  try {
    ...
    await Message.create({
      ...
      type: 'text',
      content: { 
        body: text || (templateName ? `[Template: ${templateName}] Automated message sent.` : 'Automated Template Message Sent') 
      },
    ...
    // Update conversation: update preview and move to read IF not intervened
    const existingConv = await WhatsAppConversation.findOne({ businessId, leadId });
    const currentStatus = existingConv?.status || 'unread';
    const nextStatus = currentStatus === 'intervened' ? 'intervened' : 'read';
