...
export const prepareWhatsAppMessage = (lead, business, template) => {
  ...
  const renderedMessage = (template || '').replace(/{{name}}/g, lead.name || 'Customer');
...
export const sendAutoWhatsApp = async (lead, business, template, templateName = null, headerMedia = null) => {
  ...
  try {
    if (provider === 'interakt') {
      ...
      return await sendMetaMessage(lead, business, template, templateName, headerMedia);
    }
  } catch (error) {
 ...
 */
async function sendMetaMessage(lead, business, template, templateName, headerMedia) {
  const credentials = business.integrationCredentials.whatsapp;
  ...
  if (templateName) {
    ...
    // Default: add name as first body parameter
    payload.template.components.push({
      type: "body",
      parameters: [{ type: "text", text: lead.name || 'Customer' }]
    });
  } else {
    payload = {
      messaging_product: "whatsapp",
...
async function recordOutgoingMessage(leadId, businessId, text, externalId) {
  try {
    ...
    await Message.create({
      leadId, businessId, direction: 'outgoing', text,
      externalMessageId: externalId, timestamp: new Date()
    });
