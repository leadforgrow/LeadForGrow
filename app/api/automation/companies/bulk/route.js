...
export const POST = withTenantAuth(async (request) => {
  try {
    ...
    switch (action) {
      case 'delete':
        await Company.updateMany(query, { deletedAt: new Date(), updatedBy: tenant.user._id });
        break;
      case 'archive':
        await Company.updateMany(query, { archived: true, updatedBy: tenant.user._id });
        break;
      case 'assignOwner':
        if (!data?.ownerId) {
          return NextResponse.json({ success: false, error: 'ownerId required' }, { status: 400 });
        }
        await Company.updateMany(query, { ownerId: data.ownerId, updatedBy: tenant.user._id });
        break;
      case 'addTags':
        if (!data?.tags?.length) {
          return NextResponse.json({ success: false, error: 'tags required' }, { status: 400 });
        }
        await Company.updateMany(query, { $addToSet: { tags: { $each: data.tags } }, updatedBy: tenant.user._id });
        break;
      default:
