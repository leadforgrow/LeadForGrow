  ...
  return (
    <>
      ...
      <aside
        ...
        className={`flex flex-col h-screen flex-shrink-0 z-50 bg-white dark:bg-slate-950 border-r border-slate-200/80 dark:border-slate-800 transition-[width,transform] duration-300 ease-out ${
          sidebar.isMobile
      ...
      >
        ...
        <nav
          className={`flex-1 overflow-y-auto overflow-x-hidden py-3 space-y-4 scrollbar-thin ${
            showRail ? 'px-2' : 'px-2'
          } [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-thumb]:bg-slate-200 dark:[&::-webkit-scrollbar-thumb]:bg-slate-700 [&::-webkit-scrollbar-thumb]:rounded-full`}
        >
          {groups.map((group) => (
            <SidebarSection
              key={group.id}
        ...
        <div className="flex-shrink-0 border-t border-slate-200/80 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
          <WorkspaceSwitcher
      ...
      {sidebar.isMobile && !sidebar.mobileOpen && (
        <button
          ...
          className="fixed top-3.5 left-3.5 z-40 w-9 h-9 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg flex items-center justify-center text-slate-600 shadow-md hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors lg:hidden"
